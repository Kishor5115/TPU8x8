...
puts "  - 1× RM_IHPSG13_1P_256x64_c2_bm_bist (Weight)"
puts "  - 1× RM_IHPSG13_1P_256x64_c2_bm_bist (Data)"
puts ""
puts "Output Memory:"
puts "  - 6× RM_IHPSG13_1P_64x64_c2_bm_bist"
puts "  - 3 banks × 2 SRAMs per bank (128-bit split to 2×64)"
puts ""
puts "Total: 8 SRAM macros (2 input + 6 output) - 7 KB Total"
puts ""
...
puts "INFO: Checking for existing SDC constraints..."

if {[file exists ${OUT_DIR}/${DESIGN}.sdc]} {
    puts "INFO: ${OUT_DIR}/${DESIGN}.sdc already exists. Skipping generation to avoid overwriting manual edits."
} else {
    ...
    puts $sdc_fh ""
    puts $sdc_fh "set_units -time ns -resistance kOhm -capacitance pF -voltage V -current uA"
    puts $sdc_fh ""
    ...
    puts $sdc_fh "set non_clock_inputs \[all_inputs -no_clocks\]"
    puts $sdc_fh "set signal_inputs {}"
    puts $sdc_fh "foreach p \$non_clock_inputs {"
    puts $sdc_fh "    set pname \$p"
    puts $sdc_fh "    catch {set pname \[get_name \$p\]}"
    puts $sdc_fh "    catch {set pname \[get_property \$p name\]}"
    puts $sdc_fh "    if {!\[regexp -nocase {(vdd|vss|iovdd|iovss|gnd|ring)} \$pname\]} {"
    puts $sdc_fh "        lappend signal_inputs \$p"
    puts $sdc_fh "    }"
    puts $sdc_fh "}"
    puts $sdc_fh "set signal_outputs {}"
    puts $sdc_fh "foreach p \[all_outputs\] {"
    puts $sdc_fh "    set pname \$p"
    puts $sdc_fh "    catch {set pname \[get_name \$p\]}"
    puts $sdc_fh "    catch {set pname \[get_property \$p name\]}"
    puts $sdc_fh "    if {!\[regexp -nocase {(vdd|vss|iovdd|iovss|gnd|ring)} \$pname\]} {"
    puts $sdc_fh "        lappend signal_outputs \$p"
    puts $sdc_fh "    }"
    puts $sdc_fh "}"
    puts $sdc_fh "if {\[llength \$signal_inputs\] > 0} {"
    puts $sdc_fh "    set_input_delay -clock clk_pad [expr $CLK_PERIOD * 0.2] \$signal_inputs"
    puts $sdc_fh "}"
    puts $sdc_fh "if {\[llength \$signal_outputs\] > 0} {"
    puts $sdc_fh "    set_output_delay -clock clk_pad [expr $CLK_PERIOD * 0.2] \$signal_outputs"
    puts $sdc_fh "}"
    puts $sdc_fh ""
    puts $sdc_fh "# IHP SG13G2 style global DRV constraints"
    puts $sdc_fh "set_max_fanout 8 \[current_design\]"
    puts $sdc_fh "set_max_capacitance 0.5 \[current_design\]"
    puts $sdc_fh "set_max_transition 3 \[current_design\]"
    puts $sdc_fh ""
    puts $sdc_fh "# Clock quality constraints"
    puts $sdc_fh "set_clock_uncertainty 0.15 \[get_clocks clk_pad\]"
    puts $sdc_fh "set_clock_transition 0.25 \[get_clocks clk_pad\]"
    puts $sdc_fh ""
    puts $sdc_fh "# IO assumptions"
    puts $sdc_fh "if {\[llength \$signal_inputs\] > 0} {"
    puts $sdc_fh "    set_driving_cell -lib_cell sg13g2_IOPadIn -pin pad \$signal_inputs"
    puts $sdc_fh "}"
    puts $sdc_fh "if {\[llength \$signal_outputs\] > 0} {"
    puts $sdc_fh "    set_load 0.1 \$signal_outputs"
    puts $sdc_fh "}"
    puts $sdc_fh ""
    puts $sdc_fh "# Override unrealistic macro pin defaults (A_DOUT)"
    puts $sdc_fh "set_max_capacitance 0.5 \[get_pins -hier *u_sram_data*/A_DOUT*\]"
    puts $sdc_fh "set_max_capacitance 0.5 \[get_pins -hier *u_sram_weight*/A_DOUT*\]"
    puts $sdc_fh ""
...
puts "  - 2× RM_IHPSG13_1P_256x64_c2_bm_bist (Weight + Data)"
puts "  - 6× RM_IHPSG13_1P_64x64_c2_bm_bist (Output A/B/C × 2)"
